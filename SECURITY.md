...
- Steps to reproduce (a source or toolkit ID and minimal input is ideal).
- Any relevant logs (with secrets redacted).

## Scope & handling secrets

These sources and toolkits handle credentials you supply (API keys and OAuth
...
- Secrets are sourced at runtime (macOS Keychain for sources, `ctx.secret` for
  toolkits) and must never be logged or bundled.
