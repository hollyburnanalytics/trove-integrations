...
/**
 * The two fixture rows this file writes into the library.
 *
 * @typedef {{ pk: number, subscribed: boolean, title?: string }} PodcastFixture
 * @typedef {{ podcast: number, date: string, guid?: string, uuid?: string, title?: string,
 *   webpageUrl?: string, enclosureUrl?: string, freeEnclosureUrl?: string }} EpisodeFixture
 */
 ...
 * @returns {number} Seconds since the Apple epoch.
 */
...
/**
 * An ISO instant `days` in the past.
 *
 ...
 *
 * @param {string} root - The temporary library root.
 * @param {object} [rows] - What to write.
 * @param {PodcastFixture[]} [rows.podcasts] - The shows.
 * @param {EpisodeFixture[]} [rows.episodes] - Their episodes.
 * @returns {void} Nothing; it writes the database.
 */
...
describe('apple-podcasts source', () => {
  ...
  /**
   * A context pointed at this test's temporary library.
   *
  ...
  function makeContext(overrides = {}) {
    return makeSyncContext({ config: { libraryRoot: root }, ...overrides });
  }
  ...
  it('emits recent episodes of subscribed shows as audio documents', async () => {
    ...
    expect(new Date(String(document.date)).getTime()).toBeCloseTo(
  ...
  it('resumes strictly after the date watermark and advances it to the newest emitted episode', async () => {
    ...
    expect(new Date(dateCursorValue(result.cursor)).getTime()).toBeCloseTo(
  ...
  it('caps a large backlog per run, reporting the remainder and holding the cursor at the last emitted episode', async () => {
    ...
    expect(at(first.documents, 0).title).toBe('Episode 0');
    expect(at(first.documents, 24).title).toBe('Episode 24');
  ...
  it('falls back to the episode UUID and a default title when feed metadata is missing', async () => {
    ...
    expect(at(result.documents, 0).id).toBe(stableId('apple-podcasts', 'uuid-1'));
    expect(at(result.documents, 0).title).toBe('Untitled episode');
    expect(at(result.documents, 0).url).toBe('https://a/1.mp3');
  });

  it('tolerates a show without a title and names skipped episodes by guid', async () => {
    makeLibrary(root, {
      podcasts: [{ pk: 1, subscribed: true }],
      episodes: [
    ...
    expect(result.documents).toHaveLength(1);
    expect(at(result.documents, 0).author).toBeUndefined();
