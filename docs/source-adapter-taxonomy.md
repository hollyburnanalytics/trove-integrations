# Source Adapter Taxonomy & MVP Scope

> **The normative type system for source adapters.** This is the formalization
> the manifests are validated against (`scripts/validate-registry.mjs`): every
> source declares four orthogonal type fields, and the validator enforces the
...
the axes that actually vary across source adapters:
...
- **On-demand fetch / query** — agent- or user-triggered retrieval of a single record by
  ...
  a `fetch()` / `query()` entrypoint surfaced as an MCP tool, not a source the scheduler
  ticks.
- **Snapshot** — current-state sources (price / assessment / inventory) that want **upsert**
  ...
  `INSERT … ON CONFLICT(feed_id, external_id) DO NOTHING`, which would skip changed content
...
- **The cursor is a typed `Watermark` value** (JSON), advanced by the harness via
  ...
  count) for display.
- **`source → feed → document`.** Multi-stream sources (e.g. an `rss-feeds` source
  with several feeds) are modeled as multiple feeds, each with its own cursor. Fan-out
  *across streams* is handled; fan-out *within one run* (paged-scroll) is not checkpointed.
- **The four type fields travel with the source instance**, so every layer that lists or
  schedules sources can see a source's shape — not just its name and icon.
